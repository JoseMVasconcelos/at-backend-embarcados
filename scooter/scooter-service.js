...
var db = new sqlite3.Database(`${process.env.PWD}/scooter/dados.db`, (err) => {
  if (err) {
...
app.post("/scooters", (req, res, next) => {
  ...
  db.run(
    `INSERT INTO scooter (serial_number, longitude, latitude) VALUES(?,?,?)`,
    [req.body.serial_number, req.body.longitude, req.body.latitude],
    (err) => {
      if (err) {
...
app.get("/scooters/available", (req, res, next) => {
  ...
  const params = [
    ...
    maxDistanceInKm /
      (111.32 * Math.cos(parseFloat(latitude) * (Math.PI / 180))),
    parseFloat(longitude),
    maxDistanceInKm /
      (111.32 * Math.cos(parseFloat(latitude) * (Math.PI / 180))),
  ];
...
app.get("/scooters/:serial_number", (req, res, next) => {
  db.get(
    `SELECT * FROM scooter WHERE serial_number = ?`,
